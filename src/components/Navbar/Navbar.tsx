...
const linksList = () => {
  return links.map((link) => (
    <li key={link.href} className="list-none text-xl px-4 py-2 cursor-pointer hover:text-secondary">
  ...
  return (
    <nav className="navbar flex justify-center w-full">
      <div className="navbar-container relative flex justify-between items-center w-full max-w-screen-xl px-4 py-2">
        ...
        <ul className="links sm:flex hidden">{linksList()}</ul>
        <div className="hamburger-menu sm:hidden">
          <button
            className="rounded-full p-2 hover:text-secondary"
            onClick={() => setToggle(!toggle)}
          >
            {toggle ? <IoMdClose size={40} /> : <HiMenu size={40} />}
          </button>
        </div>
        <div className="links-dropdown absolute grid justify-center right-4 -bottom-96 bg-gray-700 w-60 h-96 rounded-xl">
          {linksList()}
        </div>
      </div>
    </nav>
